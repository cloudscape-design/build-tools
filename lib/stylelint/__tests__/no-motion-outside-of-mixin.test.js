...
describe("No motion outside of mixin", () => {
  ...
  describe.each(["animation", "transition"])("%s property", property => {
    ...
    test("should allow motion behind sass control flow inside the mixin", async () => {
      const result = await runPlugin(css`
        .styled-circle-motion {
          @include styles.with-motion {
            @if $condition {
    ...
    test("should not allow motion behind sass control flow outside the mixin", async () => {
      const result = await runPlugin(css`
        .block {
          @if $condition {
            ${property}: my-animation;
    ...
    test("should not allow a nested rule between the control flow and the motion", async () => {
      const result = await runPlugin(css`
        .block {
          @include styles.with-motion {
            @if $condition {
